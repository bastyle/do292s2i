 ...
 * unrealistic and irrational way. It emits the new data on the `market` address
 * on the event bus.
 */
public class MarketDataVerticle extends AbstractVerticle {
    ...
    /**
     * @return a json representation of the market data (quote). The structure is
     *         close to <a href=
     *         "https://en.wikipedia.org/wiki/Market_data">https://en.wikipedia.org/wiki/Market_data</a>.
     */
    private JsonObject toJson() {
        return new JsonObject().put("exchange", "Vert.x stock exchange").put("symbol", symbol).put("name", name)
                .put("bid", bid).put("ask", ask).put("volume", stocks).put("open", price).put("shares", share);
