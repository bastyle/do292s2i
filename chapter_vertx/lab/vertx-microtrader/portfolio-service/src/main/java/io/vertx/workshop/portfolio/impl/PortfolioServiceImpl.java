...
public class PortfolioServiceImpl implements PortfolioService {
    ...
    public void getPortfolio(Handler<AsyncResult<Portfolio>> resultHandler) {
        // Call the given handler with a successful Async Result encapsulating the
        // `portfolio` object
    ...
    private void sendActionOnTheEventBus(String action, int amount, JsonObject quote, int newAmount) {
        // Broadcast a JSON message to the `EVENT_ADDRESS` containing the following
        // keys: "action", "quote", "date"
        // (use System.currentTimeMillis()), "amount" and "owned" (newAmount)

        // TODO: sendActionOnTheEventBus
        vertx.eventBus().publish(EVENT_ADDRESS, new JsonObject().put("action", action).put("quote", quote)
                .put("date", System.currentTimeMillis()).put("amount", amount).put("owned", newAmount));
    ...
    private Single<Double> getValueForCompany(WebClient client, String company, int numberOfShares) {
        ...
        return client.get("/?name=" + encode(company)).as(BodyCodec.jsonObject()).rxSend().map(HttpResponse::body)
                .map(json -> json.getDouble("bid")).map(val -> val * numberOfShares);
